...
import { BulkInsertEntity, DeleteEntity, GetAllEntityByType, GetEntities, GetEntitiesByCondition, InsertEntity, UpdateEntity } from "../services/services";
...
export const updateProduct = async (product: IProduct): Promise<IProduct[]> => {
    ...
    const availableProducts = await GetAllEntityByType(EntityType.PRODUCT);
...
export const getAllProducts = async () => {
    const ProductsList = await GetAllEntityByType(EntityType.PRODUCT);
    if (!ProductsList || ProductsList.length === 0) {
        throw new Error("No products found");
    }
    return mapEntityListToProductList(ProductsList);
};
